...
typedef struct {
    ...
    int assigned_to; // indice della persona assegnata (inserita per la funzione ACTbestPlan)
} Attivita;

typedef struct {
    char succ[MAXC]; // nome dell’attività che dipende
    int preced[MAXC];   // nome dell’attività da cui dipende
} Dipendenza;
